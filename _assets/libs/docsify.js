(function () {
  ...
  var nav$1 = {};
  var hoverOver$1 = false;
  var coverHeight$1 = 0;

  function highlight$1(path) {

    var sidebar = getNode('.toc-nav');
    var anchors = findAll('.anchor');
    var wrap = find(sidebar, '.page_toc');
    var active = find(sidebar, 'div.active');
    var doc = document.documentElement;
    var top = ((doc && doc.scrollTop) || document.body.scrollTop) - coverHeight$1;
    var last;
    ...
    var li = nav$1[getNavKey$1(path, last.getAttribute('data-id'))];
    ...
    // Scroll into view
    // https://github.com/vuejs/vuejs.org/blob/master/themes/vue/source/js/common.js#L282-L297
    if (!hoverOver$1 && body.classList.contains('sticky')) {
      var height = sidebar.clientHeight;
      var curOffset = 0;
      var cur = active.offsetTop + active.clientHeight + 40;
      var isInView =
  ...
  function getNavKey$1(path, id) {
    return ((decodeURIComponent(path)) + "?id=" + (decodeURIComponent(id)));
  }

  function scrollActiveSidebar$1(router) {
    var sidebar = getNode('.page_toc', true);

    var lis = [];
    if (sidebar !== null && sidebar !== undefined) {
      lis = findAll(sidebar, 'div');
    }
    ...
    for (var i = 0, len = lis.length; i < len; i += 1) {
      ...
      if (href !== '/') {
        ...
        var path$1 = ref.path;
        if (id) {
          href = getNavKey$1(path$1, id);
        }
      }

      if (href) {
        nav$1[decodeURIComponent(href)] = li;
      }
    }
    ...
    var path = removeParams(router.getCurrentPath());
    off('scroll', function () { return highlight$1(path); });
    on('scroll', function () { return highlight$1(path); });
    on(sidebar, 'mouseover', function () {
      hoverOver$1 = true;
    });
    on(sidebar, 'mouseleave', function () {
  ...
  function main(config) {
    ...
    return (
      ...
      '</section>' +
      '<aside class="toc-nav"><div class="page_toc"><!--rightToc--></div></aside>' +
      '</main>'
    );
  }
   ...
   * @return {String} Rendered tree
   */
  function treeToc(toc, tpl, maxLevel) {
    if ( tpl === void 0 ) tpl = '{inner}';
    ...
    toc.forEach(function (node) {
      ...
      innerHTML += "<div class=\"lv" + (node.level) + "\"><a href=\"" + (node.slug) + "\" data-id=\"" + title + "\" class=\"anchor\"><span>" + (node.title) + "</span></a>" + n + "</div>";
      if (node.children) {
        innerHTML += treeToc(node.children, tpl);
      }
    });
    return tpl.replace('{inner}', innerHTML);
  }
  ...
  ********************************************** */
  ...
  Compiler.prototype.tocSidebar = function tocSidebar (level) {
    if (!level) {
      this.tocr = [];
      return;
    }
    ...
    var tree = genTree(tocr, level);
    this.tocr = [];
    return !!treeToc(tree) ? '<div class="mb-2 text-bold lv2">On This Page</div>' + treeToc(tree) + '<div class="lv3 color-bg-inset pt-1 float-right" style="width: calc(100% - 20px);"></div>' : treeToc(tree);
  };
  ...
  function Render(Base) {
    return /*@__PURE__*/(function (Base) {
      ...
      Render.prototype._renderSidebar = function _renderSidebar (text) {
        ...
        this._renderTo('.page_toc', this.compiler.tocSidebar(6));
      ...
      Render.prototype._bindEventOnRendered = function _bindEventOnRendered (activeEl) {
        ...
        scrollActiveSidebar$1(this.router);

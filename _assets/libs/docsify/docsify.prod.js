(function () {
    ...
    function tinydate(str, custom) {
        ...
        str.replace(RGX, function (key, _, idx) {
            ...
            parts.push(custom && custom[key] || function (d) {
                return ('00' + (typeof MAP[key] === 'string' ? d[MAP[key]]() : MAP[key](d))).slice(
                    -key.length);
            });
        });
    ...
    var Tweezer = function () {
        ...
        _createClass$1(Tweezer, [{
            ...
            value: function _tick(currentTime) {
                ...
                this.next = this.ease(this.timeElapsed, this.start, this.end - this.start, this
                    .duration);
    ...
    function cleanUrl(sanitize, base, href) {
        if (sanitize) {
            ...
            if (prot.indexOf('javascript:') === 0 || prot.indexOf('vbscript:') === 0 || prot.indexOf('data:') ===
                0) {
                return null;
            }
        }
    ...
    var Tokenizer = /*@__PURE__*/ (function () {
        ...
        Tokenizer.prototype.html = function html(src) {
            ...
            if (cap) {
                return {
                    ...
                    text: this.options.sanitize ? (this.options.sanitizer ? this.options.sanitizer(cap[
                        0]) : escape$1(cap[0])) : cap[0]
                };
            }
        };
        ...
        Tokenizer.prototype.strong = function strong(src, maskedSrc, prevChar) {
            ...
            if (match && (!match[1] || (match[1] && (prevChar === '' || this.rules.inline.punctuation.exec(
                    prevChar))))) {
                maskedSrc = maskedSrc.slice(-1 * src.length);
                var endReg = match[0] === '**' ? this.rules.inline.strong.endAst : this.rules.inline.strong
                    .endUnd;
    ...
    var Lexer = /*@__PURE__*/ (function () {
        ...
        Lexer.prototype.inlineTokens = function inlineTokens(src, tokens, inLink, inRawBlock) {
                ...
                if (links.length > 0) {
                    while ((match = this.tokenizer.rules.inline.reflinkSearch.exec(maskedSrc)) != null) {
                        if (links.includes(match[0].slice(match[0].lastIndexOf('[') + 1, -1))) {
                            maskedSrc = maskedSrc.slice(0, match.index) + '[' + repeatString$1('a',
                                match[0].length - 2) + ']' + maskedSrc.slice(this.tokenizer.rules.inline
            ...
            while ((match = this.tokenizer.rules.inline.blockSkip.exec(maskedSrc)) != null) {
                maskedSrc = maskedSrc.slice(0, match.index) + '[' + repeatString$1('a', match[0].length -
                    2) + ']' + maskedSrc.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);
            }
                                ...
                                if (item.task) {
                                    ...
                                    if (loose) {
                                        if (item.tokens.length > 0 && item.tokens[0].type === 'text') {
                                            ...
                                            if (item.tokens[0].tokens && item.tokens[0].tokens.length >
                                                0 && item.tokens[0].tokens[0].type === 'text') {
                                                item.tokens[0].tokens[0].text = checkbox + ' ' + item.tokens[
                                                    0].tokens[0].text;
                                            }
                                        } else {
                                            item.tokens.unshift({
        ...
        Parser.prototype.parseInline = function parseInline(tokens, renderer) {
            ...
            for (i = 0; i < l; i++) {
                ...
                switch (token.type) {
                    ...
                    case 'link':
                        {
                            out += renderer.link(token.href, token.title, this.parseInline(token.tokens,
                                renderer));
                            break;
                        }
                    case 'image':
    ...
    function corner(data, cornerExternalLinkTarget) {
        ...
        return (
            "<a href=\"" + data + "\" target=\"" + cornerExternalLinkTarget +
    ...
    function main(config) {
        ...
        var aside =
            ...
            '<aside class="sidebar">' +
            (config.name ?
                ("<h1 class=\"app-name\"><a class=\"app-name-link\" data-nosearch>" + (config.logo ? ("<img alt=\"" +
                    name + "\" src=" + (config.logo) + ">") : name) + "</a></h1>") :
                '') +
    ...
    function tree(toc, tpl) {
        ...
        toc.forEach(function (node) {
            ...
            innerHTML += "<li><a class=\"section-link\" href=\"" + (node.slug) + "\" title=\"" + title +
                "\">" + (node.title) + "</a></li>";
            if (node.children) {
    ...
    function replaceEmojiShorthand(m, $1, useNativeEmoji) {
        ...
        if (emojiMatch) {
            if (useNativeEmoji && /unicode/.test(emojiMatch)) {
                ...
                result = "<img src=\"" + (emojiData.baseURL) + emojiMatch + ".png\" alt=\"" + $1 +
    ...
    var imageCompiler = function (ref) {
        ...
        return (renderer.image = function (href, title, text) {
            ...
            if (attrs.length > 0) {
                return ("<img src=\"" + url + "\" data-origin=\"" + href + "\" alt=\"" + text + "\" " +
                    (attrs.join(
                        ' '
                    )) + " />");
            }
        ...
        var Prism = (function (_self) {
            ...
            var _ = {
                ...
                highlightAllUnder: function (container, async, callback) {
                    ...
                    env.elements = Array.prototype.slice.apply(env.container.querySelectorAll(
                        env.selector));
                ...
                highlightElement: function (element, async, callback) {
                    ...
                    if (parent && parent.nodeName.toLowerCase() === 'pre' && !parent.hasAttribute(
                            'tabindex')) {
                ...
                for (var token in grammar) {
                    ...
                    for (var j = 0; j < patterns.length; ++j) {
                        ...
                        for ( // iterate the token list and keep track of the current token/string position
                            var currentNode = startNode.next, pos = startPos; currentNode !==
                            tokenList.tail; pos += currentNode.value.length, currentNode =
                            currentNode.next
                        ) {
                            ...
                            if (greedy) {
                                ...
                                for (
                                    var k = currentNode; k !== tokenList.tail && (p < to || typeof k
                                        .value === 'string'); k = k.next
                                ) {
                                    removeCount++;
                                    p += k.value.length;
                                }
        ...
        ********************************************** */
        ...
        (function (Prism) {
            ...
            Prism.languages.css = {
                ...
                'selector': {
                    pattern: RegExp('(^|[{}\\s])[^{}\\s](?:[^{};"\'\\s]|\\s+(?![\\s{])|' + string.source +
                        ')*(?=\\s*\\{)'),
                    lookbehind: true
                },
                'string': {
                ...
                value: function (env, language) {
                    ...
                    function walkTokens(tokens) {
                        for (var i = 0; i < tokens.length; i++) {
                            ...
                            var token = tokens[i];
                            if (typeof token === 'string' || (token.content && typeof token.content ===
                                    'string')) {
                                var k = keys[j];
    ...
    var highlightCodeCompiler = function (ref) {
        ...
        return (renderer.code = function (code, lang) {
            ...
            return ("<pre v-pre data-lang=\"" + lang + "\"><code class=\"lang-" + lang + "\">" + text +
                "</code></pre>");
        });
    };
    ...
    Compiler.prototype._initRenderer = function _initRenderer() {
        ...
        origin.heading = renderer.heading = function (text, level) {
            ...
            return ("<h" + level + " id=\"" + slug + "\"><a href=\"" + url + "\" data-id=\"" + slug +
                "\" class=\"anchor\"><span>" + str + "</span></a></h" + level + ">");
        };
        ...
        if (text) {
            ...
            for (var i = 0; i < toc.length; i++) {
                if (toc[i].ignoreSubHeading) {
                    ...
                    for (
                        var j = i; j < toc.length && deletedHeaderLevel < toc[j].level; j++
                    ) {
                        toc.splice(j, 1) && j-- && i++;
                    }
